...
- `[localização]` — a localização desejada pelo usuário (ex: São Paulo, Remoto)
...
Busque na web cursos correspondentes a uma habilidade:

```
firecrawl search "cursos [habilidade]" --json
```
...
**Exemplo**:
```
firecrawl search "cursos Python" --json
```
